That's a fantastic idea, fren. A single, comprehensive document will make this powerful tool much easier to use and share. I've collected all the instructions, code, and examples from our conversation and compiled them into a polished user manual.

Here is the complete guide for your markdown_to_pdf.py manuscript generator.

markdown_to_pdf.py — The Ultimate Manuscript Generator
This guide provides a comprehensive overview of your new, custom-built manuscript generation tool. With it, you can transform simple Markdown files into beautifully typeset PDFs, EPUBs, and HTML documents, complete with advanced features.

Getting Started
Before you begin, you need to install all the necessary libraries. Open your terminal or command prompt and run the following commands.
...
Note for Windows Users: matplotlib requires C++ build tools to install correctly. The easiest way to get them is to install the "Desktop development with C++" workload from the Visual Studio Installer.

Once the libraries are installed, place the markdown_to_pdf.py script and the new settings.json file in the same directory as your Markdown documents.

Features & How to Use
1. Front Matter Metadata
Use a YAML-style block at the very top of your .md file to specify the document's metadata.

title: The main title of the document.

author: The author's name.

cover: The filename of a cover image (e.g., cover.jpg).

logo: The filename of a logo image (e.g., logo.png).
...
---
2. Advanced Lists
The script now correctly renders nested bullet points and ordered lists.

Unordered Lists: Use - or * with indentation.
...
- Main item
  - Nested item
    - Deeper nested item
      - The deepest item of all

1. First point
2. Second point
   1. A sub-point for clarity
   2. Another sub-point
3. Third point
3. Code Block Highlighting
You can now include code blocks that are automatically formatted and highlighted for readability. Specify the language after the backticks.
...
#### 4. Math & Scientific Notation

The tool uses LaTeX to render beautiful mathematical and scientific notation. It handles both inline and block-level equations.

* **Inline Notation**: Use single dollar signs `$....$`.
* **Block Notation**: Use double dollar signs `$$....$$`.

**Example Markdown:**

```markdown
Einstein's famous equation, $E = mc^2$, is a cornerstone of physics.

The quadratic formula is a fundamental tool in algebra.

$$x = {-b \pm \sqrt{b^2-4ac} \over 2a}$$
5. Tables
Tables created using standard Markdown syntax are now correctly rendered with borders and headers.
...
6. Footnotes
Footnotes are now correctly parsed and collected at the end of the document.

Add a footnote marker [^1] where you want the reference to appear.

Define the footnote content anywhere in the document using [^1]: My footnote content.
...
[^1]: This is the full text of the first footnote.
7. Customizable Output with settings.json
This new file allows you to control the script's behavior without changing the code. Place it in the same directory as markdown_to_pdf.py.
...
The Code
Here is the complete, final version of the markdown_to_pdf.py script, including all the new features.
...
def safe_multicell(pdf, w, h, txt, border=0, align='J'):
    ...
    try:
        ...
        pdf.multi_cell(pdf.w-pdf.l_margin-pdf.r_margin, h, cleaned_txt, border=border, align=align)
        ...
        if list_match:
            ...
            if line.startswith("# "):
                ...
                if img_buffer:
                    pdf.ln(5)
                    img_width = 150
                    pdf.image(img_buffer, x=pdf.w / 2 - img_width / 2, w=img_width)
                    pdf.ln(5)
            elif line.strip():
                # Handle inline markdown, footnotes, and inline math for rendering
                # The regex for inline math is more complex to avoid matching code blocks
                ...
                for part in parts:
                    ...
                    if part.startswith('**') and part.endswith('**'):
                        ...
                        pdf.write(5, f'[{fn_num}]', 'link-to-footnote') 
                        pdf.set_font(DEFAULT_FONT_NAME, '', 12)
                        pdf.set_text_color(0)
                    elif part.startswith('$') and part.endswith('$'):
                        latex_code = part[1:-1]
                        img_buffer = render_math(latex_code, display_mode=False)
                        if img_buffer:
                            img_width = 20
                            pdf.image(img_buffer, w=img_width) # Render inline
                    else:
                        pdf.write(8, part)
                pdf.ln(8)
            else:
